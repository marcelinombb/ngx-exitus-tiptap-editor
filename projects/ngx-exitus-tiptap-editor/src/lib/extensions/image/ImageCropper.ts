...
export default class ImageCropper {
  ...
  private async enable() {
    ...
    const width =
      this.overlay.clientWidth || this.target.image.clientWidth || fallbackWidth || MIN_CROP_SIZE;
    const height =
      this.overlay.clientHeight ||
      this.target.image.clientHeight ||
      fallbackHeight ||
      MIN_CROP_SIZE;
  ...
  private createHandles() {
    const handles: CropHandleDirection[] = [
      'topLeft',
      'topRight',
      'bottomLeft',
      'bottomRight',
  ...
  private async applyCrop() {
    ...
    try {
      ...
      window.alert(
        'Não foi possível recortar a imagem. Verifique se a imagem permite acesso (CORS) ou se o arquivo é válido.',
      );
    } finally {
      this.disable();
    }
