...
export const MyExtension = Node.create<MyNodeOptions>({
  ...
  renderHTML({ HTMLAttributes }) {
    return ['div', mergeAttributes(this.options.HTMLAttributes, HTMLAttributes, { 'data-type': 'my-extension' }), 0];
  },
...
*Note: The Injector is passed to the extension via `.configure({ injector: this.injector })` when the editor is instantiated in the main component.*
